...
run-detached:
	docker run --name php${VERSION} -d -v $(PWD):/var/www/app $(REPO):${VERSION}
	docker run --name php${VERSION}-dev -d -v $(PWD):/var/www/app $(REPO):${VERSION}-dev
test-main:
	$(DOCKER_RUN) php -v
...
release: build
	$(eval export SEMVER=$(shell docker run --rm -v $(PWD):/var/www/app ${REPO}:${VERSION} php -r "echo phpversion();"))
